...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const agreement = await p.tblAgreements.findUnique({
      where: { AgrID: id },
      include: {
        tblCustomers: { select: { CustID: true, name: true, Street: true, City: true, PostalCode: true, Country: true } },
        tblTitles: { select: { TitleID: true, Title: true, ISBN: true, AuthID: true } },
    ...
    return NextResponse.json({
      ...
      commission: agreement.Commission ?? 0,
      commissionMaterials: agreement.CommissionMaterials ?? 0,
      ...
      estimatedCopyPrice: agreement.EstimatedCopyPrice || 0,
      graalShare: agreement.GraalShare || 0,
